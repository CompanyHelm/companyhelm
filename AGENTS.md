...
- When changing CompanyHelm user-facing capabilities, navigation, setup flows, or these agent-facing repo instructions, also update the `CompanyHelm guide` system skill template in `apps/api/src/templates/system_skills/companyhelm_guide.md` if the change affects what agents should tell company users they can do in CompanyHelm.
