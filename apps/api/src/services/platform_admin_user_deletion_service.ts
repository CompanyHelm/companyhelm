...
/**
 * Deletes a product user from the platform-admin boundary by removing local memberships and the
 * local user row under the admin database role.
 */
@injectable()
export class PlatformAdminUserDeletionService {
  ...
  static createForTest(
    ...
    return new PlatformAdminUserDeletionService(adminDatabase);
  }
