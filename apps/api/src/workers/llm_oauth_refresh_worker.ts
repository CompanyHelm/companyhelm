...
type LlmOauthCredentialScope = "company" | "platform";
...
export class LlmOauthRefreshWorker extends WorkerBase {
  ...
  protected async run(): Promise<void> {
    ...
    await this.adminDatabase.getSqlClient().begin(async (transactionSql) => {
      ...
      await this.refreshCompanyCredentials(sql, refreshCutoff);
      await this.refreshPlatformCredentials(sql, refreshCutoff);
    });
  }
  ...
  private async refreshCompanyCredentials(sql: Sql, refreshCutoff: string): Promise<void> {
    ...
    for (const credential of credentials) {
      try {
        ...
        await this.markCompanyCredentialRefreshFailed(sql, credential, error);
      }
    }
  }
  ...
  private async refreshPlatformCredentials(sql: Sql, refreshCutoff: string): Promise<void> {
    ...
    for (const credential of credentials) {
      try {
        ...
        await this.markPlatformCredentialRefreshFailed(sql, credential, error);
      }
    }
  }

  private async markCompanyCredentialRefreshFailed(
    sql: Sql,
    credential: LlmOauthCredentialRow,
    error: unknown,
  ): Promise<void> {
    ...
    await sql`
            ...
            WHERE "id" = ${credential.id}
          `;
    this.logRefreshFailure("company", credential, message);
  }

  private async markPlatformCredentialRefreshFailed(
    sql: Sql,
    credential: LlmOauthCredentialRow,
    error: unknown,
  ): Promise<void> {
    ...
    await sql`
            ...
            WHERE "id" = ${credential.id}
          `;
    this.logRefreshFailure("platform", credential, message);
  }

  private logRefreshFailure(
    credentialScope: LlmOauthCredentialScope,
    credential: LlmOauthCredentialRow,
    message: string,
  ): void {
