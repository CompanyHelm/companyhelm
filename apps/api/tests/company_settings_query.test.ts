...
class CompanySettingsQueryTestHarness {
                ...
                return {
                  async where() {
                    return [{
                      ...
                      baseSystemPrompt: "Always keep responses aligned with company goals.",
                    }];
                  },
                };
              },
...
test("GraphQL CompanySettings query returns the persisted company prompt layer", async () => {
  const app = Fastify();
  const config = CompanySettingsQueryTestHarness.createConfigMock();
  const database = CompanySettingsQueryTestHarness.createDatabaseMock();
  const modelManager = {
  ...
  assert.deepEqual(document.data.CompanySettings, {
    companyId: "company-123",
    baseSystemPrompt: "Always keep responses aligned with company goals.",
  });
