        ...
        return {
          values(value: Record<string, unknown> | Array<Record<string, unknown>>) {
            ...
            if (table === modelProviderCredentials) {
              throw new Error("Company bootstrap must not provision model provider credentials.");
            }
...
test("CompanyBootstrapService does not duplicate onboarding assets when rerun", async () => {
  ...
  const harness = new CompanyBootstrapServiceTestHarness({
    ...
    modelCredentialRows: [{
      companyId: "company-1",
      createdAt: now,
      encryptedApiKey: "encrypted-openai-api-key",
      id: "openai-model-credential-1",
      isDefault: true,
    ...
    modelRows: [{
      ...
      isDefault: true,
      modelId: "gpt-5.4",
      modelProviderCredentialId: "openai-model-credential-1",
      name: "GPT-5.4",
      reasoningLevels: ["low", "medium", "high", "xhigh"],
