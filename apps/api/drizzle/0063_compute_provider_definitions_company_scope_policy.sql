...
BEGIN
  FOR policy_record IN
    ...
    FROM pg_policies
    WHERE schemaname = 'public'
      AND tablename = 'daytona_compute_provider_definitions'
  LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON "daytona_compute_provider_definitions"', policy_record.policyname);
  END LOOP;
END
$$;
--> statement-breakpoint
...
ON "daytona_compute_provider_definitions"
AS PERMISSIVE
FOR ALL
TO public
USING (
  EXISTS (
    ...
    WHERE "cpd"."id" = "daytona_compute_provider_definitions"."compute_provider_definition_id"
