...
import type { agentArchivedChatsTabQuery } from "./__generated__/agentArchivedChatsTabQuery.graphql";
...
const ARCHIVED_CHATS_PAGE_SIZE = 40;
const ARCHIVED_CHATS_LOAD_MORE_SCROLL_MARGIN = 240;

const agentArchivedChatsTabQueryNode = graphql`
  query agentArchivedChatsTabQuery($agentId: ID!, $first: Int!, $after: String) {
...
type ArchivedSessionsConnection = agentArchivedChatsTabQuery["response"]["ArchivedAgentSessions"];

function toSessionRecords(connection: ArchivedSessionsConnection | null | undefined): AgentArchivedChatsTabSessionRecord[] {
  return (connection?.edges ?? []).map((edge) => ({
    ...edge.node,
    associatedTask: edge.node.associatedTask ?? null,
    inferredTitle: edge.node.inferredTitle ?? null,
    lastUserMessageAt: edge.node.lastUserMessageAt ?? null,
    userSetTitle: edge.node.userSetTitle ?? null,
  }));
}

function mergeSessionRecords(
  currentSessions: AgentArchivedChatsTabSessionRecord[],
  nextSessions: AgentArchivedChatsTabSessionRecord[],
): AgentArchivedChatsTabSessionRecord[] {
  const mergedSessionsById = new Map(currentSessions.map((session) => [session.id, session]));
  for (const session of nextSessions) {
    mergedSessionsById.set(session.id, session);
  }

  return [...mergedSessionsById.values()];
}
...
export function AgentArchivedChatsTab(props: AgentArchivedChatsTabProps) {
  ...
  const archivedChatsData = useLazyLoadQuery<agentArchivedChatsTabQuery>(
    agentArchivedChatsTabQueryNode,
    {
      agentId: props.agentId,
      after: null,
      first: ARCHIVED_CHATS_PAGE_SIZE,
    },
    {
      fetchPolicy: "store-and-network",
    },
  );
  const initialConnection = archivedChatsData.ArchivedAgentSessions;
  const scrollContainerRef = useRef<HTMLDivElement | null>(null);
  const isLoadingMoreRef = useRef(false);
  const [selectedSessionIds, setSelectedSessionIds] = useState<string[]>([]);
  const [loadedSessions, setLoadedSessions] = useState<AgentArchivedChatsTabSessionRecord[]>(
    () => toSessionRecords(initialConnection),
  );
  const [pageInfo, setPageInfo] = useState({
    endCursor: initialConnection.pageInfo.endCursor ?? null,
    hasNextPage: initialConnection.pageInfo.hasNextPage,
  });
  ...
  useEffect(() => {
    setLoadedSessions(toSessionRecords(initialConnection));
    setPageInfo({
      endCursor: initialConnection.pageInfo.endCursor ?? null,
      hasNextPage: initialConnection.pageInfo.hasNextPage,
    });
  ...
  const visibleSessions = loadedSessions.filter((session) => !removedSessionIds.includes(session.id));
  ...
  const loadMoreArchivedChats = useCallback(async () => {
    if (isLoadingMoreRef.current || !pageInfo.hasNextPage || !pageInfo.endCursor) {
      return;
    }
    ...
    try {
      const response = await fetchQuery<agentArchivedChatsTabQuery>(
        relayEnvironment,
        agentArchivedChatsTabQueryNode,
        {
          agentId: props.agentId,
      ...
      const connection = response?.ArchivedAgentSessions;
      setLoadedSessions((currentSessions) => {
        return mergeSessionRecords(currentSessions, toSessionRecords(connection));
      });
      setPageInfo({
      ...
      setErrorMessage(error instanceof Error ? error.message : "Failed to load archived chats.");
    } finally {
      isLoadingMoreRef.current = false;
      setIsLoadingMore(false);
    }
  }, [pageInfo.endCursor, pageInfo.hasNextPage, props.agentId, relayEnvironment]);

  const checkArchivedChatsScroll = useCallback(() => {
    const scrollContainer = scrollContainerRef.current;
    if (!scrollContainer) {
      return;
    }

    const remainingScrollDistance = scrollContainer.scrollHeight
      - scrollContainer.scrollTop
      - scrollContainer.clientHeight;
    if (remainingScrollDistance <= ARCHIVED_CHATS_LOAD_MORE_SCROLL_MARGIN) {
      void loadMoreArchivedChats();
    }
  }, [loadMoreArchivedChats]);

  useEffect(() => {
    checkArchivedChatsScroll();
  }, [checkArchivedChatsScroll, visibleSessions.length]);
  ...
  return (
    <Card variant="page" className="flex min-h-[calc(100vh-13rem)] flex-1 flex-col overflow-hidden rounded-2xl border border-border/60 shadow-sm">
      <CardHeader>
      ...
      <CardContent className="flex min-h-0 flex-1 flex-col gap-4">
        ...
        {visibleSessions.length === 0 ? (
          <div className="flex min-h-0 flex-1 flex-col items-center justify-center rounded-xl border border-dashed border-border/70 bg-muted/20 px-4 py-10 text-center">
          ...
          <div
            ...
            className="min-h-0 flex-1 overflow-y-auto rounded-md border border-border/60"
            onScroll={checkArchivedChatsScroll}
          >
            <Table>
                ...
                <TableRow>
                  <TableHead className="w-12">
                    <SelectionCheckbox
                      ...
                      label="Select loaded archived chats"
                      onChange={() => {
                        setSelectedSessionIds((currentSessionIds) => {
              ...
              <TableBody>
                ...
                {pageInfo.hasNextPage || isLoadingMore ? (
                  <TableRow>
                    <TableCell className="py-4 text-center text-xs text-muted-foreground" colSpan={6}>
                      {isLoadingMore ? "Loading more archived chats..." : "Scroll for more archived chats"}
                    </TableCell>
                  </TableRow>
                ) : null}
              </TableBody>
